...
class Environment:
    ...
    def get_position_bytes(self, state):
        return [state[70], state[72], state[99], state[101]]
    ...
    def run_episode(self, epsilon):
        ...
        while not done:
            ...
            if not done:
                self.qTable.update_q(current_discrete_state, action, new_discrete_state, self.get_reward(new_state))
            else:
                print("Score", self.get_reward(new_state))
